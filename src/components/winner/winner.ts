...
export class Winner extends BaseComponent {
  constructor(number:number,color:string,name:string,wins:number,time:number) {
    super("div", ["winners_table__winner"]);
    const carModel = this.renderCar(color);

    this.element.innerHTML = ` 
        <div class="winner_number">${number}</div>
        <div class="winner_model">${carModel}</div>
        <div class="winner_name">${name}</div>
        <div class="winner_wins">${wins}</div>
        <div class="winner_best-time">${time}</div>
    `;
  }
